...
export default async (req: Request, res: Response) => {
    ...
    if (!req.body.target) {
        return res.json({error: 'target must not be empty'})
    }

    if (req.body.target.includes('\'')) {
        return res.status(400).json({error: 'Invalid target'})
    }

    if (!guild.members.includes(req.body.target)) {
        return res.status(401).json({error: 'Target must be exists in guild'})
    }

    const result = (await req.shard.broadcastEval(`const djs = require('discord.js');this.guilds.cache.get('${req.query.id}')?.members.cache.get('${req.body.target}')?.send(
