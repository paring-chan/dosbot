...
export default async (req: Request, res: Response) => {
    ...
    if (!req.body.role) {
        return res.json({error: 'role must not be empty'})
    }
    ...
    if (!guild.roles.includes(req.body.role)) {
        return res.status(401).json({error: 'Target role must be exists in guild'})
    }
    ...
    ).then(async m => m.react('✅'))`)).filter(r => r)
